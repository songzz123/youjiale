...
use app\hayao\model\HyaoAbapAiLog;
use app\hayao\model\HyaoAnswers;
use app\hayao\model\HyaoChildren;
use app\hayao\model\HyaoGrowup;
use app\hayao\model\HyaoKupperMan;
use think\Request;

class Hayao extends Frontend
{
    ...
    public function ajaxAction()
    {
        ...
        switch ($act) {
            ...
            case 'gailv':
                ...
                $hasOne = HyaoAnswers::where($firstWhere)->find();
                ...
                if ($hasOne) {
                    ...
                    $upbool = HyaoAnswers::update($qai2, ["session_id" => $data['session_id']]);
    ...
    public function testFunc()
    {
        ...
        if ($request->isAjax()) {
            ...
            if (!empty($age) && !empty($height) && !empty($weight)) {
                ...
                $listresult = HyaoGrowup::where($ageWhere)
                    ->field($field)
                    ->find();
    ...
    public function getTop($zz, $zz_height, $mbi, $session_id, $sex, $age)
    {
        ...
        $where['sex'] = $sex;
        $where['age'] = $age;
        $testResult = HyaoChildren::where($where)
            ->field($field)
            ->order('height DESC')
            ->select();
    ...
    public function fileput()
    {
        ...
        if (isset($_POST) and $_SERVER['REQUEST_METHOD'] == "POST") {
            ...
            if (move_uploaded_file($tmp, $path . $name)) {
                if (HyaoGrowup::getExcel($path . $name, $type)) {
                    $this->success('上传成功');
                }
            } else {
